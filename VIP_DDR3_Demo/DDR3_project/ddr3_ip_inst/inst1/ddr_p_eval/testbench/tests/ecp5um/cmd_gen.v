...
begin
   ...
   check_bl_cl_wr (bl, cl, `inst1_TWR*`inst1_X_FAC, bl_bits, cl_bits, wr_bits);
   addr[1:0]   = bl_bits;
   addr[2]     = cl_bits[0];
   addr[3]     = bt;
   ...
   addr[11:9]  = wr_bits;
   addr[12]    = 0;  //Slow exit for Precharge_inst1 Powerdown(DLL off)
...
begin
   ...
   addr[`inst1_COL_WIDTH-1:0]                           = col_addr;
`ifdef inst1_CS_WIDTH_1
   addr[`inst1_BSIZE+`inst1_COL_WIDTH-1:`inst1_COL_WIDTH]           = bank_addr;
`else
   addr[`inst1_BSIZE+`inst1_COL_WIDTH-1:`inst1_COL_WIDTH]           = {cs_addr,bank_addr[2:0]};
`endif
   addr[`inst1_ROW_WIDTH+`inst1_BSIZE+`inst1_COL_WIDTH-1:`inst1_BSIZE+`inst1_COL_WIDTH]     = row_addr;
   ofly_burst_len = otf;

   `ifndef inst1_NO_INFO
   $display ("%t TB INFO: Read from Bank No.%5d, Row Addr :%5d, Col Addr: %5d", $time, bank_addr, row_addr, col_addr); 
   `endif

`ifdef inst1_CMD_VALID_TYPE1
...
//             If opted, deassert all user side signals
   if (!cmd_valid_lat) begin
       ...
       #1;
       cmd_valid = 1'b0;
      `ifdef inst1_CLOSE_USR_SIG
        ff_burst_count   = 0;
        addr[`inst1_COL_WIDTH-1:0]                      = 0;
        addr[`inst1_BSIZE+`inst1_COL_WIDTH-1:`inst1_COL_WIDTH]      = 0;
        addr[`inst1_ROW_WIDTH+`inst1_BSIZE+`inst1_COL_WIDTH-1:`inst1_BSIZE+`inst1_COL_WIDTH] = 0;
        ofly_burst_len = 1;
      `endif
...
`ifdef inst1_X4_GEAR   // In x4 mode only BL8 is supported
      case (bl_in)
         4 : bl_out = 2'b00;  // bl 8
         8 : bl_out = 2'b00;  // bl 8
         0 : bl_out = 2'b00;  // bl 8 
...
//---------------- Task generate_data --------------------------------
task generate_data;
output [`inst1_DSIZE+(`inst1_DSIZE/8)-1:0] data_out ;
reg    [`inst1_DSIZE+(`inst1_DSIZE/8)-1:0] data_out ;
reg    [`inst1_DSIZE-1:0]            int_data_out ;
reg    [7:0]                   data8   [35:0];
reg    [`inst1_USER_DM-1:0]          dm_bits;
