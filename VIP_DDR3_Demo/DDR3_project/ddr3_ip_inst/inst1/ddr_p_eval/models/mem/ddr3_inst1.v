...
/*
`ifdef inst1_den1024Mb
    `include "1024Mb_ddr3_parameters.vh"
`elsif inst1_den2048Mb
    `include "2048Mb_ddr3_parameters.vh"
`elsif inst1_den4096Mb
...
*/
    ...
    // text macros
    `define inst1_DQ_PER_DQS DQ_BITS/DQS_BITS
    `define inst1_BANKS (1<<BA_BITS)
    `define inst1_MAX_BITS (BA_BITS+ROW_BITS+COL_BITS-BL_BITS)
    `define inst1_MAX_SIZE (1<<(BA_BITS+ROW_BITS+COL_BITS-BL_BITS))
    `define inst1_MEM_SIZE (1<<MEM_BITS)
    `define inst1_MAX_PIPE 4*CL_MAX
    ...
    // Mode Registers
    reg     [ADDR_BITS-1:0] mode_reg [`inst1_BANKS-1:0];
    reg     burst_order;
    ...
    // command state
    reg     [`inst1_BANKS-1:0] active_bank;
    reg     [`inst1_BANKS-1:0] auto_precharge_bank;
    reg     [`inst1_BANKS-1:0] write_precharge_bank;
    reg     [`inst1_BANKS-1:0] read_precharge_bank;
    reg     [ROW_BITS-1:0] active_row [`inst1_BANKS-1:0];
    reg     in_power_down;
    reg     in_self_refresh;
    ...
    integer ck_cke_cmd;
    integer ck_bank_write     [`inst1_BANKS-1:0];
    integer ck_bank_read      [`inst1_BANKS-1:0];
    ...
    time    tm_cke_cmd;
    time    tm_ttsinit;
    time    tm_bank_precharge [`inst1_BANKS-1:0];
    time    tm_bank_activate  [`inst1_BANKS-1:0];
    time    tm_bank_write_end [`inst1_BANKS-1:0];
    time    tm_bank_read_end  [`inst1_BANKS-1:0];
    time    tm_group_activate  [1:0];
    time    tm_group_write_end [1:0];

    // pipelines
    reg     [`inst1_MAX_PIPE:0]  al_pipeline;
    reg     [`inst1_MAX_PIPE:0]  wr_pipeline;
    reg     [`inst1_MAX_PIPE:0]  rd_pipeline;
    reg     [`inst1_MAX_PIPE:0]  odt_pipeline;
    reg     [`inst1_MAX_PIPE:0]  dyn_odt_pipeline;
    reg     [BL_BITS:0]    bl_pipeline  [`inst1_MAX_PIPE:0];
    reg     [BA_BITS-1:0]  ba_pipeline  [`inst1_MAX_PIPE:0];
    reg     [ROW_BITS-1:0] row_pipeline [`inst1_MAX_PIPE:0];
    reg     [COL_BITS-1:0] col_pipeline [`inst1_MAX_PIPE:0];
    reg     prev_cke;
...
`ifdef inst1_MAX_MEM
    ...
    begin : file_io_open
        ...
        for (bank = 0; bank < `inst1_BANKS; bank = bank + 1)
            memfd[bank] = open_bank_file(bank);
    end
`else
    reg     [BL_MAX*DQ_BITS-1:0] memory  [0:`inst1_MEM_SIZE-1];
    reg     [`inst1_MAX_BITS-1:0]      address [0:`inst1_MEM_SIZE-1];
...
`ifdef inst1_MAX_MEM
    ...
    function integer_inst1 open_bank_file( input integer bank );
        integer fd;
        reg [2048:1] filename;
        begin 
...
`else
            ...
            end else if (memory_used == `inst1_MEM_SIZE) begin
...
`endif
    ...
    task erase_banks;
        input  [`inst1_BANKS-1:0] banks; //one select bit per bank
        reg [BA_BITS-1:0] ba;
        reg [`inst1_MAX_BITS-1:0] i;
        integer bank;
                    ...
                    PRECHARGE : begin
                        ...
                        if (|active_bank) begin
                            ...
                            if (mpr_en) begin
                                ...
                                for (i=0; i<`inst1_BANKS; i=i+1) begin
    ...
    task data_task;
        ...
        begin
            ...
            if (dq_in_valid) begin // write data
                ...
                if (diff_ck) begin
                    for (i=0; i<DM_BITS; i=i+1) begin
                        bit_mask = bit_mask | ({`inst1_DQ_PER_DQS{~dm_in_neg[i]}}<<(burst_position*DQ_BITS + i*`inst1_DQ_PER_DQS));
                    end
                    memory_data = (dq_in_neg<<(burst_position*DQ_BITS) & bit_mask) | (memory_data & ~bit_mask);
                end else begin
                    for (i=0; i<DM_BITS; i=i+1) begin
                        bit_mask = bit_mask | ({`inst1_DQ_PER_DQS{~dm_in_pos[i]}}<<(burst_position*DQ_BITS + i*`inst1_DQ_PER_DQS));
                    end
            ...
            if (rdq_cntr > 0) begin // read data
                if (mpr_en) begin
`ifdef inst1_MPR_DQ0 // DQ0 output MPR data, other DQ low				   
                    if (mpr_select == 2'b00) begin // Calibration Pattern
                        dq_temp = {DQS_BITS{{`inst1_DQ_PER_DQS-1{1'b0}}, calibration_pattern[burst_position]}};
                    end else if (odts_readout && (mpr_select == 2'b11)) begin // Temp_inst1 Sensor (ODTS)
                        dq_temp = {DQS_BITS{{`inst1_DQ_PER_DQS-1{1'b0}}, temp_sensor[burst_position]}};
                    end else begin // Reserved
                        dq_temp = {DQS_BITS{{`inst1_DQ_PER_DQS-1{1'b0}}, 1'bx}};
                    end
`else // all DQ output MPR data
                    if (mpr_select == 2'b00) begin // Calibration Pattern
                        dq_temp = {DQS_BITS{{`inst1_DQ_PER_DQS{calibration_pattern[burst_position]}}}};
                    end else if (odts_readout && (mpr_select == 2'b11)) begin // Temp_inst1 Sensor (ODTS)
                        dq_temp = {DQS_BITS{{`inst1_DQ_PER_DQS{temp_sensor[burst_position]}}}};
                    end else begin // Reserved
                        dq_temp = {DQS_BITS{{`inst1_DQ_PER_DQS{1'bx}}}};
                    end
`endif				   
            ...
            if (RANDOM_OUT_DELAY && (dqs_out_en || (|dqs_out_en_dly) || dq_out_en || (|dq_out_en_dly))) begin
                for (i=0; i<DQS_BITS; i=i+1) begin
                    ...
                    if (!write_levelization) begin
                        for (j=0; j<`inst1_DQ_PER_DQS; j=j+1) begin
                            dq_out_en_dly[i*`inst1_DQ_PER_DQS + j] <= #(tck_avg/2) dq_out_en;
                            if (dqsq_min == dqsq_max) begin
                                dq_out_dly   [i*`inst1_DQ_PER_DQS + j] <= #(tck_avg/2 + dqsq_min) dq_out[i*`inst1_DQ_PER_DQS + j];
                            end else begin
                                dq_out_dly   [i*`inst1_DQ_PER_DQS + j] <= #(tck_avg/2 + $dist_uniform(seed, dqsq_min, dqsq_max)) dq_out[i*`inst1_DQ_PER_DQS + j];
                            end
                        end
                    end
                end
...
`endif
    ...
    always @(negedge rst_n_in or posedge diff_ck or negedge diff_ck) begin : main
        ...
        if (!rst_n_in) begin
            ...
            if (|wr_pipeline || |rd_pipeline || |al_pipeline) begin
                ...
                for (i=0; i<`inst1_MAX_PIPE; i=i+1) begin
    ...
    task dqs_even_receiver;
        ...
        reg [127:0] bit_mask;
        begin
            bit_mask = {`inst1_DQ_PER_DQS{1'b1}}<<(i*`inst1_DQ_PER_DQS);
            if (dqs_even[i]) begin
    ...
    begin
        if (dqs_in_valid) begin
            if ($time - tm_dqs[i/`inst1_DQ_PER_DQS] < TDH) 
                $display ("%m: at time %t ERROR:   tDH violation on DQ bit %d by %t", $time, i, tm_dqs[i/`inst1_DQ_PER_DQS] + TDH - $time);
    ...
    begin
        if (write_levelization && i<32) begin
            ...
            dq_out_en_dly[i*`inst1_DQ_PER_DQS] <= #(TWLO) 1'b1;
            dq_out_dly[i*`inst1_DQ_PER_DQS] <= #(TWLO) diff_ck;
`ifdef inst1_WL_ALLDQ
            for (j=1; j<`inst1_DQ_PER_DQS; j=j+1) begin
                dq_out_en_dly[i*`inst1_DQ_PER_DQS+j] <= #(TWLO) 1'b1;
                dq_out_dly[i*`inst1_DQ_PER_DQS+j] <= #(TWLO) diff_ck;
            end
`else
            for (j=1; j<`inst1_DQ_PER_DQS; j=j+1) begin
                dq_out_en_dly[i*`inst1_DQ_PER_DQS+j] <= #(TWLO + TWLOE) 1'b1;
                dq_out_dly[i*`inst1_DQ_PER_DQS+j] <= #(TWLO + TWLOE) 1'b0;
            end
`endif
        end
        if (dqs_in_valid && ((wdqs_pos_cntr[i] < wr_burst_length/2) || b2b_write)) begin
            if (dqs_in[i] ^ prev_dqs_in[i]) begin
                ...
                if (!dq_out_en) begin
                    for (j=0; j<`inst1_DQ_PER_DQS; j=j+1) begin
                        if ($time - tm_dq[(i%32)*`inst1_DQ_PER_DQS+j] < TDS) 
                            $display ("%m: at time %t ERROR: tDS violation on DQ bit %d by %t", $time, i*`inst1_DQ_PER_DQS+j, tm_dq[(i%32)*`inst1_DQ_PER_DQS+j] + TDS - $time);
                        check_dq_tdipw[(i%32)*`inst1_DQ_PER_DQS+j] <= 1'b1;
                    end
                end
